...
import { FileText, FolderTree, ScrollText } from 'lucide-react'
    ...
    <section className="flex h-full min-h-0 flex-col bg-surface-base/80">
      ...
      <div
        className={cn(
          ...
          viewMode === 'focused' && 'xl:grid-cols-[280px_minmax(0,1fr)]',
        )}
      >
        {viewMode === 'focused' ? (
            ...
            manuscript.sections.length > 0 ? (
              <div className="mx-auto w-full max-w-4xl">
                <motion.div
                  ...
                  transition={reducedMotion ? undefined : { duration: 0.2, ease: [0.2, 0, 0, 1] }}
                  className="space-y-6"
                >
                  {manuscript.sections.flatMap((section) =>
                    section.files.map((file) => (
                      <div key={file.id} id={createFileAnchorId(file.path)}>
                        {file.review ? (
