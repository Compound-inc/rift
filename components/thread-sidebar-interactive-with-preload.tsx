...
interface ThreadSidebarInteractiveWithPreloadProps {
  preloadedThreads?: Preloaded<typeof api.threads.getUserThreadsPaginated>;
}

export function ThreadSidebarInteractiveWithPreload({
  preloadedThreads,
}: ThreadSidebarInteractiveWithPreloadProps) {
  console.log(
    "🔄 ThreadSidebarInteractiveWithPreload render - preloaded:",
    !!preloadedThreads,
  );
  return (
    <>
      {preloadedThreads ? (
        <PreloadedContent preloadedThreads={preloadedThreads} />
      ) : (
        <FallbackContent />
      )}
    </>
...
function PreloadedContent({
  ...
  console.log("🔄 PreloadedContent render");
  const preloadedResults = usePreloadedQuery(preloadedThreads);
  console.log("📊 Raw preloadedResults:", preloadedResults);
  ...
  const status = preloadedResults.isDone ? "Exhausted" : "CanLoadMore";
  const loadMore = (numItems: number) => {
    return fallbackResults.loadMore(numItems);
  };
  ...
  return (
    <ThreadContent threads={threads} status={status} loadMore={loadMore} />
  );
}

function FallbackContent() {
  console.log("🔄 FallbackContent render");
  const {
    results: threads = [],
    status,
    loadMore,
  ...
  console.log("📋 FallbackContent threads:", threads.length, "status:", status);
            ...
            {filteredThreads.map((thread) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className="absolute top-0 bottom-0 right-0 opacity-0 group-hover:opacity-100 transition-all duration-200 ease-out flex items-center gap-1 bg-white rounded-l-md p-1 translate-x-full group-hover:translate-x-0 shadow-container-small"
                      ...
                      >
                        <g clipPath="url(#clip0_10_1140)">
                          ...
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M25 18.9296C25.8284 18.9296 26.5 19.6012 26.5 20.4296V32.4296C26.5 33.2582 25.8284 33.9296 25 18.9296C24.1716 18.9296 23.5 19.6012 23.5 20.4296V32.4296C23.5 33.2582 24.1716 33.9296 25 33.9296Z"
                            fill="currentColor"
                          />
                          <path
                            fillRule="evenodd"
