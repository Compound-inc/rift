...
export function MembersList({
  ...
  const [memberToUpdate, setMemberToUpdate] =
    useState<OrganizationMembershipWithUser | null>(null);
  ...
  const [invitationToRevoke, setInvitationToRevoke] =
    useState<WorkOSInvitation | null>(null);
  const [isRevoking, setIsRevoking] = useState(false);
  const [revokeError, setRevokeError] = useState<string | null>(null);

  const isLimitReached =
    typeof seatQuantity === "number" && totalMemberCount >= seatQuantity;
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const validInvitations = invitationForms.filter(
        (inv) => inv.email.trim() !== "",
      );
      ...
      if (typeof seatQuantity === "number") {
        ...
        if (validInvitations.length > availableSeats) {
          setInviteError(
            `No puedes enviar ${validInvitations.length} invitaciones. Solo tienes ${availableSeats} asiento${availableSeats !== 1 ? "s" : ""} disponible${availableSeats !== 1 ? "s" : ""}.`,
          );
          setIsInviting(false);
          return;
        }
      ...
      const results = await Promise.all(
        validInvitations.map((inv) => inviteUser(inv.email, inv.role)),
      );
      ...
      if (failures.length === 0) {
        ...
        if (firstError && firstError.includes("Email already invited")) {
          ...
          setInviteError(
            `Error al enviar la invitación: ${firstError || "Error desconocido"}`,
          );
        }
      }
    } catch (error) {
      console.error(error);
      setInviteError(
        "Ocurrió un error inesperado al procesar las invitaciones.",
      );
    } finally {
      setIsInviting(false);
    }
  ...
  const openUpdateRoleDialog = (member: OrganizationMembershipWithUser) => {
    ...
    const roleSlug =
      typeof member.role === "object" ? member.role?.slug : member.role;
    ...
    const memberRows = data.members.map((m) => ({
      ...
      name: m.user
        ? m.user.firstName && m.user.lastName
          ? `${m.user.firstName} ${m.user.lastName}`
          : m.user.firstName || m.user.lastName || "Nombre Desconocido"
        : "Usuario Desconocido",
      email: m.user?.email || "",
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <DialogTrigger asChild>
              <Button
                variant="ghost"
                disabled={isLimitReached || basePlan !== "enterprise"}
                ...
                <div className="space-y-6 p-2">
                  <DialogHeader className="space-y-2">
                    ...
                    <DialogDescription className="text-base text-muted-foreground leading-relaxed">
                      Escribe el correo y selecciona el rol para cada nuevo
                      miembro.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <form onSubmit={handleInvite} className="space-y-4">
                    ...
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto p-1">
                      {invitationForms.map((invitation, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-[1fr_200px] gap-4 items-start"
                        >
                          <Input
                          ...
                          >
                            ...
                            <SelectContent className="min-w-[190px] rounded-2xl border border-border/60 bg-white/95 p-1.5 shadow-xl shadow-black/10 backdrop-blur-sm dark:bg-popover-secondary/85">
                              <SelectItem
                                value="member"
                                className="cursor-pointer rounded-xl px-3 py-2 mb-1 text-sm font-medium text-foreground/80 hover:bg-black/5] dark:hover:bg-hover/40"
                              >
                                Miembro
                              </SelectItem>
                              <SelectItem
                                value="admin"
                                className="cursor-pointer rounded-xl px-3 py-2 text-sm font-medium text-foreground/80 hover:bg-black/5 dark:hover:bg-hover/40"
                              >
                                Administrador
                              </SelectItem>
                            </SelectContent>
                    ...
                    <div className="pt-2">
                      <Button
                        ...
                        disabled={
                          ...
                          (typeof seatQuantity === "number" &&
                            totalMemberCount + invitationForms.length >=
                              seatQuantity)
                        }
              ...
              <div className="space-y-3">
                ...
                <Select value={newRole} onValueChange={setNewRole}>
                  ...
                  <SelectContent className="min-w-[190px] rounded-2xl border border-border/60 bg-white/95 p-1.5 shadow-xl shadow-black/10 backdrop-blur-sm dark:bg-popover-secondary/85">
                    <SelectItem
                      value="member"
                      className="cursor-pointer rounded-xl px-3 py-2 mb-1 text-sm font-medium text-foreground/80 hover:bg-black/[0.04] dark:hover:bg-hover/40"
                    >
                      Miembro
                    </SelectItem>
                    <SelectItem
        ...
        <DialogContent className="max-w-md rounded-2xl border border-border/50 bg-white/95 dark:bg-popover-main shadow-2xl">
          <div className="space-y-6 p-2">
            <DialogHeader className="space-y-2">
              ...
              <DialogDescription className="text-base text-muted-foreground leading-relaxed">
                ¿Estás seguro que deseas revocar la invitación para{" "}
            ...
            {loading ? (
              <TableRow>
                <TableCell colSpan={6} className="h-24 text-center">
                  <ReloadIcon className="mr-2 size-4 animate-spin inline" />{" "}
                  Cargando...
                </TableCell>
              </TableRow>
            ) : (
              <>
                {processedData.map((item, index) => {
                  ...
                  const isCurrentUser = item.isInvitation
                    ? false
                    : item.rawMember?.userId === currentUserId;
                  ...
                  return (
                    <TableRow
                      ...
                      className={cn(
                        ...
                        index % 2 === 0
                          ? "bg-black/0 dark:bg-transparent"
                          : "bg-black/[0.015] dark:bg-transparent",
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            ...
                            rolePillStyles[(role ?? "default").toLowerCase()] ??
                              rolePillStyles.default,
                          )}
                        >
                          {getRoleLabel(role)}
                        </Badge>
                          ...
                          >
                            {item.isInvitation ? (
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  openRevokeDialog(item.rawInvitation!)
                                }
                              >
                                Revocar invitación
                              </DropdownMenuItem>
                              ...
                              <>
                                <DropdownMenuItem
                                  className="cursor-pointer rounded-xl px-3 py-2 text-sm font-medium text-foreground/80 hover:bg-black/[0.04] dark:hover:bg-hover/40"
                                  onClick={() =>
                                    openUpdateRoleDialog(item.rawMember!)
                                  }
                                >
                                  Actualizar rol
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  ...
                                  className="cursor-pointer rounded-xl px-3 py-2 text-sm font-medium text-destructive hover:bg-destructive/10 focus:text-destructive"
                                  onClick={() =>
                                    openRemoveDialog(item.rawMember!)
                                  }
                                >
                                  Eliminar miembro
                                </DropdownMenuItem>
