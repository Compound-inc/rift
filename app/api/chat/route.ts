...
const jsonResponse = (body: object, status: number, start: number): Response =>
...
const errorToResponse = (
  ...
  switch (error._tag) {
    ...
    case "ToolError":
      logger.error(`Tool error: ${error.message}`, logContext, {
        cause: error.cause,
      });
      return makeResponse(
...
const handleChatRequest = (
  ...
  Effect.gen(function* () {
    ...
    logger.debug("Request validated", logContext, {
      trigger,
    ...
    const rest: Effect.Effect<Response, ChatRouteError, Scope.Scope> =
      Effect.gen(function* () {
        const customInstructionsContent = customInstruction?.instructions;
        const validatedCustomInstructionId = customInstruction
          ? customInstructionId
          : undefined;
        ...
        const userFiles =
          lastUser?.parts?.filter((part) => part.type === "file") || [];
        const modelConfig = getModel(modelId);
        const supportsImageOutput =
          modelConfig?.capabilities.supportsImageOutput ?? false;
        ...
        const supermemoryEnabled =
          Boolean(process.env.SUPERMEMORY_API_KEY) &&
          userConfig.supermemoryEnabled;
        ...
        const phClient = yield* Effect.try({
          try: () =>
            ...
            logger.warn(
              "PostHog initialization failed, analytics disabled",
              logContext,
              error,
            );
            return null;
        ...
        const model = supportsImageOutput
          ...
          : yield* Effect.try({
              try: () => {
                if (phClient) {
                  return withTracing(
                    modelWithProvider as any as Parameters<
                      typeof withTracing
                    >[0],
                    phClient,
                    {
        ...
        if (lastUser && (userText || userFiles.length > 0)) {
          ...
          if (trigger !== "regenerate-message") {
            const attachmentIds = userFiles
              .filter(
                (part): part is typeof part & { attachmentId: string } =>
                  "attachmentId" in part &&
                  typeof (part as { attachmentId?: string }).attachmentId ===
                    "string",
              )
              .map(
                (part) =>
                  (part as { attachmentId: string })
                    .attachmentId as Id<"attachments">,
              );
            ...
            yield* sendUserMessage({
              ...
              Effect.tapError((error) =>
                Effect.sync(() => {
                  logger.error(
                    `Failed to save user message: ${idempotencyKey}`,
                    logContext,
                    error,
                  );
                }),
        ...
        const stream = createUIMessageStream({
          ...
          execute: async ({ writer }) => {
            ...
            Effect.runPromise(startEffect).catch((err) =>
              logger.error(
                "Failed to enqueue startAssistantMessage",
                logContext,
                err,
              ),
            );
            ...
            const flushUpdate = async () => {
              ...
              if (update.content.length > 0 || update.reasoning.length > 0) {
                ...
                Effect.runPromise(appendEffect).catch((err) =>
                  logger.error(
                    "Failed to enqueue appendMessageDelta",
                    logContext,
                    err,
                  ),
                );
              ...
              Effect.runPromise(Ref.get(stateRef)).then(async (state) => {
                ...
                if (finalState.searchToolCallCount > 0) {
                  ...
                  try {
                    ...
                    logger.error(
                      "Failed to track tool quota (Autumn)",
                      logContext,
                      err,
                    );
                  }
            ...
            logger.debug("Starting AI stream", logContext, {
              timeMs: Date.now() - start,
            });
            ...
            if (supportsImageOutput) {
              try {
                ...
                const imageModel = phClient
                  ? (withTracing(
                      getLanguageModel(modelId) as any as Parameters<
                        typeof withTracing
                      >[0],
                      phClient,
                      {
                ...
                await Effect.runPromise(
                  ...
                  logger.error(
                    "Failed to finalize image error",
                    logContext,
                    finalizeErr,
                  ),
                );
            ...
            try {
              ...
              const result = streamText({
                ...
                onChunk: ({ chunk }) => {
                  ...
                  if (chunk.type === "text-delta" && chunk.text) {
                    ...
                    if (SEARCH_TOOLS.has(chunk.toolName)) {
                      ...
                      logger.debug(
                        "Non-search tool call (not counted for quota)",
                        logContext,
                        {
                          toolName: chunk.toolName,
                        },
                    ...
                    if (Array.isArray(toolResult)) {
                      ...
                      if (sources.length > 0) {
                        Effect.runPromise(
                          ...
                          logger.error(
                            "Failed to enqueue addSourcesToMessage",
                            logContext,
                            err,
                          ),
                        );
                ...
                onFinish: async () => {
                  ...
                  if (validatedCustomInstructionId) {
                    try {
                      await fetchMutation(
                        api.customInstructions.serverIncrementUsage,
                        {
                      ...
                      logger.warn(
                        "Failed to increment custom instruction usage",
                        logContext,
                        err,
                      );
                    }
                ...
                onError: async (error) => {
                  ...
                  if (isAbort) {
                    ...
                    logger.info("Stream aborted", logContext, {
                      contentLength: finalState.content.length,
                    });
              ...
              try {
                ...
                console.error(
                  "Failed to capture stream execution error to Sentry",
                  sentryError,
                );
              }
...
export async function POST(req: Request): Promise<Response> {
  ...
  const program = Effect.gen(function* (_) {
    const mainFiber = yield* _(
      Effect.forkScoped(handleChatRequest(req, requestId)),
    );
    ...
    Effect.catchTags({
      ValidationError: (e: ValidationError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      RegenerateError: (e: RegenerateError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      ...
      NoOrganizationError: (e: NoOrganizationError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      NoSubscriptionError: (e: NoSubscriptionError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      SeatLimitError: (e: SeatLimitError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      QuotaExceededError: (e: QuotaExceededError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
      AbortError: (e: AbortError) =>
        Effect.succeed(errorToResponse(e, start, requestId, logContext)),
    ...
    Effect.catchAll((error: unknown) => {
      ...
      try {
        ...
        console.error(
          "Failed to capture unhandled error to Sentry",
          sentryError,
        );
      }
